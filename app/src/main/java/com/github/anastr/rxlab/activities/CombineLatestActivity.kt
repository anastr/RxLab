...
class CombineLatestActivity: OperationActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setCode("Observable o1 = Observable.interval(1500, 3000, TimeUnit.MILLISECONDS);\n" +
        ...
        surfaceView.addDrawingObject(observableOperation1)
        val observableOperation2 = TextOperation("interval", "3000")
        surfaceView.addDrawingObject(observableOperation2)
        val combineLatestOperation = FixedEmitsOperation("combineLatest", ArrayList())
        surfaceView.addDrawingObject(combineLatestOperation)
